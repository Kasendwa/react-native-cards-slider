...
class CardSilder extends Component {
	...
	componentDidUpdate(prevProps, prevState) {
		if (prevState.position !== this.state.position && typeof this.props.onPositionChanged === 'function') {
	...
	render() {
		...
		return (
			<ScrollView {...this.props} ref={slider => (this.slider = slider)} style={[styles.scroll, this.props.style]} onScroll={e => this.scroll(e.nativeEvent)} horizontal={true} pagingEnabled={true} showsHorizontalScrollIndicator={false} scrollEventThrottle={20}>
				{cards}
			</ScrollView>
		);
	}
